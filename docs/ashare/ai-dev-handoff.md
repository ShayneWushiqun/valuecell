...
- `python/valuecell/server/services/assets/decision_outcome_review_service.py`
  - 已提供决策结果回看 MVP，复用日线历史价格、决策记录、时间窗上下文和当日快照做 5 / 10 / 20 日轻量回看
...
- 已完成决策结果回看 MVP，支持基于日线价格的 5 / 10 / 20 日轻量、保守、可解释 outcome review
- 已完成决策上下文页 MVP，补齐“当前为什么这样判断”的独立解释页
- 还没有完成更细粒度提醒去重与历史运营能力和自动交易
- 部分规则仍是轻量版，需要继续校准字段口径和阈值
- 还没有完成分钟级实时事件流、全市场事件总线、复杂收益归因和回测
- 还没有接真实 LLM 做结果回看或自动交易
