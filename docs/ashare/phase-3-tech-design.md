...
- `HoldingLifecycleService`
  - 复用现有持仓、持仓动作、提醒与首页市场语境，给每只持仓归类到固定生命周期阶段
- `ExitRiskCenterService`
  - 复用现有持仓动作结果，补高优先级处理、保护利润、纪律止损与风险桶聚合
- `DecisionRecordService`
  - 复用持仓周期、持仓动作与最近快照，把关键判断按日沉淀为可回看的决策记录

阶段三第二波已完成的能力：

- `ShortCycleContextEventService`
  - 围绕重点标的宇宙，把市场、题材、机会、提醒、持仓和记录结果归一为统一事件对象
- `DecisionContextWindowService`
  - 基于统一事件生成最近 10 / 20 / 40 日的决策时间窗，并整理支持 / 反对 / 风险证据
- `DecisionRecordService`
