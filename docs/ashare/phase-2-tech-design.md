...
- `EntryTimingService`
  - 负责基于 OpportunityPoolService 输出轻量规则版买点裁决信号
  - 只使用保守 action 表达，不输出绝对化买入结论
  - 当前仍不包含 Agent 裁决、提醒中心、自动交易或持久化
- `DecisionAlertService`
  - 负责基于 EntryTimingService 输出轻量提醒摘要
  - 将买点接近、等待确认、持有观察、风险回避等变化统一聚合为规则版提醒
  - 当前不做去重持久化、不做完整提醒中心、不接 Agent、不做自动交易
- `StrategyPreferenceService`
  - 负责读取和保存 A 股策略偏好模板
  - 当前复用 `user_profiles` 存储，使用 JSON 记录模板与偏好参数
  - 偏好只影响机会池排序、买点裁决和提醒优先级，不直接生成激进动作
- `DecisionAlertPersistenceService`
  - 负责把规则版提醒沉淀为可回看的提醒中心
  - 当前支持刷新、列表、已读、全部已读与忽略
  - 去重先按 user + ticker + alert_type + action + 当日日期做轻量处理
...
- `HoldingExitSignalService`
  - 负责基于持仓、阶段一诊断、A 股裁决上下文与规则裁决，产出持仓处理 judgement
  - 只使用继续持有、持有观察、减仓观察、保护利润、纪律止损五类动作
  - 当前不做自动交易，不把持仓判断外推成新开仓建议
...
这意味着当前状态是“阶段二 MVP 规则版闭环 + 阶段三前置上下文准备”已完成，具体包括机会池、买点裁决、轻量提醒摘要、提醒中心持久化 MVP、机会池页面、策略偏好模板、Agent 裁决上下文 MVP、Agent 裁决接口 MVP 和持仓卖点与减仓裁决 MVP；但这仍不等于“阶段三真实 Agent 裁决已经完成”。

当前仍未完成：

- 真实 LLM / Agent 裁决
- 完整复杂提醒系统
- 更细粒度的去重 / 已读 / 历史提醒持久化能力
- 自动交易

当前已覆盖三条主线：

- 买什么：机会池
- 什么时候买：买点裁决、提醒与规则 fallback 裁决
- 持仓怎么处理：持仓诊断 + 持仓卖点与减仓裁决
