...
from valuecell.adapters.assets.ashare_provider import is_ashare_ticker
...
class PaperExecutionGateway(BaseExecutionGateway):
    ...
    async def execute(
        ...
        for inst in instructions:
            ...
            if ref_price <= 0:
                results.append(
                    TxResult(
                        ...
                        reason="Missing market price for paper execution.",
            ...
            if is_ashare_ticker(inst.instrument.symbol):
                if inst.action in {
                    ...
                    results.append(
                        TxResult(
                            ...
                            filled_qty=0.0,
                            status=TxStatus.REJECTED,
                            reason="A-share paper trading does not support short selling.",
                            leverage=inst.leverage,
                            meta=inst.meta,
                        )
                    )
                ...
                normalized_qty = int(float(inst.quantity) // 100 * 100)
                if normalized_qty <= 0:
                    results.append(
                        TxResult(
                            instruction_id=inst.instruction_id,
                            ...
                            filled_qty=0.0,
                            status=TxStatus.REJECTED,
                            reason="A-share order quantity must be at least one lot (100 shares).",
                            leverage=inst.leverage,
                            meta=inst.meta,
                        )
                    )
                ...
                normalized_qty = float(inst.quantity)
            if side == TradeSide.BUY:
                exec_price = ref_price * (1.0 + slip)
            else:
                exec_price = ref_price * (1.0 - slip)

            notional = exec_price * float(normalized_qty)
            ...
            results.append(
                TxResult(
                    ...
                    side=side,
                    requested_qty=float(inst.quantity),
                    filled_qty=float(normalized_qty),
